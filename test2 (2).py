...
foodmap = [[0 for x in range(window_height)] for y in range(window_width)]
...
class Ant(pygame.sprite.Sprite):
	def __init__(self,nest_position_x,nest_position_y,color):
		...
		self.position_x = random.randint(nest_position_x,nest_position_x + nest_size - size)
		self.position_y = random.randint(nest_position_y,nest_position_y + nest_size - size)
	...
	def update(self):
		if self.health == 0 and self.state != "dead":
			...
			if pygame.sprite.spritecollide(self,food_list,False):
				...
				max_y = 0
				maxnum = 0
				for i in range(-food_odor,food_odor):
					for j in range(-food_odor,food_odor):
						if self.position_x+i>=0 and self.position_x+i <800 and self.position_y+j >=0 and self.position_y+j < 600:
							if foodmap[self.position_x+i][self.position_y+j] > maxnum:
								maxnum = foodmap[self.position_x+i][self.position_y+j]
								max_x = i
								max_y = j
				if maxnum > 0:
			...
			if pygame.sprite.spritecollide(self,nest_list,False):
				...
				for i in range(1):
					ant = Ant(self.nest_position_x,self.nest_position_y,self.search_color)
					ant_list.add(ant)
					all_list.add(ant)
